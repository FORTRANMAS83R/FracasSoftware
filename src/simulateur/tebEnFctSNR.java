	...
	public static void main(String[] args) throws Exception {
		...
		for (int j = 0; j < nombreDeSimulations; j++) {
			...
			for (double snr = snrMin; snr <= snrMax; snr += 0.1) {
				String[] argumentsNRZetNRZT = new String[] { "-mess", String.valueOf(tailleMessage), "-form", "NRZT",
						"-nbEch", String.valueOf(nbEch), "-ampl", String.valueOf(ampMinNRZetNRZT),
						String.valueOf(ampMaxNRZetNRZT), "-snrpb",
						String.valueOf(Math.round(snr * 1000) / (float) 1000) };
				String[] argumentsRZ = new String[] { "-mess", String.valueOf(tailleMessage), "-form", "NRZT", "-nbEch",
						String.valueOf(nbEch), "-ampl", String.valueOf(ampMinRZ), String.valueOf(ampMaxRZ), "-snrpb",
						String.valueOf(Math.round(snr * 1000) / (float) 1000) };
