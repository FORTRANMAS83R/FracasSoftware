...
public class Simulateur {
	...
	public Simulateur(String[] args) throws ArgumentsException {
		...
		// source = messageAleatoire ? new SourceAleatoire(nbBitsMess, seed) : new
		// SourceFixe(messageString);
		source = new SourceNRZ(messageString, 30, 0, 1);
		transmetteurLogique = new TransmetteurParfait<Float>();
		source.connecter(transmetteurLogique);
		if (affichage)
			source.connecter(new SondeAnalogique("Entrée"));
		destination = new DestinationFinale<Float>();
		transmetteurLogique.connecter(destination);
		if (affichage)
			transmetteurLogique.connecter(new SondeAnalogique("Sortie"));
	}
