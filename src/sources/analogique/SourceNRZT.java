...
public class SourceNRZT extends SourceAnalogique {
    ...
    protected void filtreMiseEnForme() {
        ...
        for (int i = 0; i < informationEchantillon.nbElements(); i += nbEchantillon) {
            ...
            final float val_fin = informationEchantillon.nbElements() > i + nbEchantillon ? (informationEchantillon.iemeElement(i + nbEchantillon) + symbole_actuel) / 2 : 0;
            final float val_depart = i == 0 ? 0 : (informationEchantillon.iemeElement(i - 1) + symbole_actuel) / 2;
            final float delta_1er_tier = symbole_actuel == val_depart ? 0 : (symbole_actuel - val_depart) / ((float) nbEchantillon / 3f);
            final float delta_3eme_tier = symbole_actuel == val_fin ? 0 : (val_fin - symbole_actuel) / (((float) nbEchantillon / 3f) - (i + nbEchantillon == informationEchantillon.nbElements() ? 1 : 0));

            if (delta_1er_tier != 0)
                for (int j = 0; j < nbEchantillon / 3; j++) {
                    informationGeneree.setIemeElement(i + j, val_depart + j * delta_1er_tier);
                }

            if (delta_3eme_tier != 0)
                for (int j = 0; j < nbEchantillon / 3; j++) {
                    informationGeneree.setIemeElement(i + (2 * nbEchantillon / 3) + j, symbole_actuel + j * delta_3eme_tier);
                }
        }
    }
}
