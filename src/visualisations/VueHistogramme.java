...
public class VueHistogramme extends Vue {
    ...
    public void paint(Graphics g) {
        ...
        getContentPane().getGraphics().drawLine(getContentPane().getWidth() - 5, y0Axe - 5, getContentPane().getWidth(),
                y0Axe);
        getContentPane().getGraphics().drawLine(getContentPane().getWidth() - 5, y0Axe + 5, getContentPane().getWidth(),
                y0Axe);
        ...
        if (histogramme.length != 0) {
            ...
            for (int i = 0; i < histogramme.length; i++) {
                getContentPane().getGraphics().drawRect((int) (x0Axe + deltaX * i),
                        (int) (y0Axe - deltaY * histogramme[i]), (int) deltaX, (int) (deltaY * histogramme[i]));
            }
        }
    }
